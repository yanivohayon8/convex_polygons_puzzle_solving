...
class TestAssemblyPlotter(unittest.TestCase):
    ...
    def test_toy_example_print_loaded_images(self):
        ...
        images = plotter._load_images([bag_of_pieces[0]])
    ...
    def test_toy_example(self):
        ...
        pieces = [bag_of_pieces[0],bag_of_pieces[1]]

        # rotation_angles = [0.0]#[-5.688992977142334] # #
        # transvectors =[(0.0,0.0)]# # #[(0,0)] #
        # pieces = [bag_of_pieces[0]]


        # transvectors = [(0.0,0.0),(-650,-900.70440673828125)]

        img = plotter.plot(rotation_angles,transvectors,pieces)
        # img = plotter.plot(rotation_angles,transvectors,centers)

        plt.imshow(img)
        plt.show()

        raise NotImplementedError("under constructoin")
