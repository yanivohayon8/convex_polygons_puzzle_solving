    ...
    def pairwise(self):
        for edge1_i in range(self.total_num_edges):
            ...
            for edge2_j in range(self.total_num_edges):
                ...
                edge1_pixels = self.pieces[piece1_i].features[self.feature_extrapolator][edge1_local_i]

                piece2_j = self.edge2piece_index[edge2_j]
                edge2_local_j = self.global_index2local_index[edge2_j]
                edge2_pixels = self.pieces[piece2_j].features[self.feature_original][edge2_local_j]
                
                self.matching_edges_scores[edge1_i,edge2_j] = self._score_pair(edge1_pixels["same"],edge2_pixels["flipped"])
